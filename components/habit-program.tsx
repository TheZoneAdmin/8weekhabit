                ...
                <div className="mt-2">
                  <div className="w-full bg-gray-600 rounded-full h-2">
                    <div
                      ...
                      style={{ width: `${Math.min(calculateAchievementProgress(achievement, savedData), 100)}%` }}
                    />
                  </div>
                  <p className="text-xs text-gray-400 mt-1 text-right">
                    {Math.round(calculateAchievementProgress(achievement, savedData))}%
                  </p>
                </div>
              )}
            </div>
...
const HabitProgram = () => {
  ...
  const handleCheckbox = useCallback((program: string, week: number, habitIndex: number, checked: boolean) => {
    const today = new Date().toISOString().split('T')[0];

    setSavedData(prev => {
      const newData = { ...prev };
      if (!newData.programs[program]) {
        newData.programs[program] = {};
      }
      if (!newData.programs[program][week]) {
        newData.programs[program][week] = {};
      }
      if (!newData.programs[program][week][habitIndex]) {
        newData.programs[program][week][habitIndex] = {
          completed: false,
          completionDates: []
        };
      }

      const currentHabit = newData.programs[program][week][habitIndex];
      ...
      if (checked) {
        ...
        currentHabit.completed = true;
        currentHabit.completionDates = updatedDates;
      } else {
        currentHabit.completed = false;
        currentHabit.completionDates = currentHabit.completionDates.filter(date => date !== today);
      }
    ...
    setUserData(prev => {
      ...
      const updatedAchievements = prev.achievements.map(achievement => {
        ...
        const progress = calculateAchievementProgress(achievement, savedData);
        if (progress >= 100) {
          return {
            ...achievement,
            unlocked: true,
    ...
    saveData();
  }, [savedData, setSavedData, setUserData, saveData]);
                      ...
                      <div key={idx} className="group">
                        <div className="flex items-center space-x-4 text-white">
                          <input 
                            ...
                            checked={savedData.programs[key]?.[week.week]?.[idx]?.completionDates?.includes(
                          ...
                          <div>
                            ...
                            <p className="text-gray-400 text-xs mt-1">
                              Completed {savedData.programs[key]?.[week.week]?.[idx]?.completionDates?.length || 0} times
                            </p>
                          </div>
                        </div>
                      </div>
